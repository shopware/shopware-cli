...
Do not turn Store documentation, schema fields, or recommendations into local validation errors unless the current CLI actually enforces them.
...
The presence of a field in the schema means the CLI supports that configuration shape. It does **not** by itself prove that the field is mandatory for local validation or Store acceptance.
...
Do not claim the normal plugin icon must be exactly 256×256. Use current validator output/source for accepted dimensions and file-size limits.

A physical `LICENSE` file or `README.md` may be useful or required by external Store policy, but do not claim either is a local CLI validation requirement unless current validation or authoritative Store documentation explicitly establishes that requirement.
...
- label them as **official Store guidance/manual review requirements** when sourced from Store documentation;
- label CLI findings as **CLI-enforced** only when a fresh run or current source proves enforcement;
...
8. Separate what the CLI enforces from what Store documentation or manual review requires.
...
Prefer version-correct evidence over remembered behavior.
