...
func Cleanup(project string) error {
	for _, file := range removeFiles {
		if _, err := os.Stat(path.Join(project, file)); os.IsNotExist(err) {
			continue
		}

		if err := os.Remove(path.Join(project, file)); err != nil {
			return err
		}
	}

	for _, directory := range removeDirectories {
		if _, err := os.Stat(path.Join(project, directory)); os.IsNotExist(err) {
			continue
		}

		if err := os.RemoveAll(path.Join(project, directory)); err != nil {
			return err
		}
	}
...
func CleanupByHash(project string) error {
	for file, md5s := range cleanupByMd5 {
		...
		if slices.Contains(md5s, md5String) {
			if err := os.Remove(path.Join(project, file)); err != nil {
				return err
			}
		}
