...
func checkServicesXmlInResourceDir(check validation.Check, resourceDir string) {
	...
	if _, err := os.Stat(servicesXml); err == nil {
		check.AddResult(validation.CheckResult{
			...
			Message:    fmt.Sprintf("Found deprecated %s. Symfony services.xml is deprecated, migrate to services.yaml. Run \"shopware-cli extension fix\" to convert it automatically.", servicesXml),
