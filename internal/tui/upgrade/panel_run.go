...
type runState struct {
	events   <-chan backend.StepEvent
	cancelFn context.CancelFunc

	stepStates map[backend.StepID]backend.CheckState
	stepErrs   map[backend.StepID]error
	logLines   []string
	fullLog    bool
...
func (m *Model) updateRun(msg tea.Msg) (app.Content, tea.Cmd) {
	switch msg := msg.(type) {
	case runEventMsg:
		ev := backend.StepEvent(msg)
		switch {
		case ev.Line != "":
			m.run.logLines = tui.AppendTail(m.run.logLines, runLogKeep, ev.Line)
		case ev.Step == backend.StepFinished:
			m.run.finished = true
			m.run.succeeded = ev.State == backend.StateOK
			m.run.err = ev.Err
		default:
...
func (m *Model) viewRunProgress() string {
	...
	for _, id := range backend.RunSteps {
