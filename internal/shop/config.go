...
type Config struct {
	...
	// Deprecated: shop URL belongs under environments. Empty -e uses environments.local. Top-level url is still read during the deprecation window.
	...
	// Deprecated: Admin API credentials belong under environments. Empty -e uses environments.local. Top-level admin_api is still read during the deprecation window.
