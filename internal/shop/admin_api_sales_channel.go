...
// their domains.
func ListStorefrontSalesChannels(ctx context.Context, client *shopware.Client) ([]SalesChannel, error) {
	repo := shopware.NewRepository[SalesChannel](client, "sales_channel")
...
func FindThemeForSalesChannel(ctx context.Context, client *shopware.Client, salesChannelID string) (*Theme, error) {
	repo := shopware.NewRepository[Theme](client, "theme")
	result, err := repo.Search(ctx, shopware.NewCriteria().
		SetLimit(1).
