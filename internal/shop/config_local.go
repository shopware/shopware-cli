...
// UpdateLocalDockerPHP merges the non-empty profiler credential fields into
// docker.php of the local override file, preserving all other content.
func UpdateLocalDockerPHP(configPath string, php *ConfigDockerPHP) error {
	...
	secrets := map[string]string{
		"blackfire_server_id":    php.BlackfireServerID,
		"blackfire_server_token": php.BlackfireServerToken,
		"tideways_api_key":       php.TidewaysAPIKey,
	}
	...
	return updateLocalConfig(configPath, func(root *yaml.Node) error {
		...
		for _, key := range []string{"blackfire_server_id", "blackfire_server_token", "tideways_api_key"} {
			if secrets[key] == "" {
				continue
			}

			setMappingValue(phpNode, key, &yaml.Node{
				Kind:  yaml.ScalarNode,
				Tag:   "!!str",
				Value: secrets[key],
			})
