...
func (m *Model) beginReview(token string, avail pluginmigrate.Availability) (app.Content, tea.Cmd) {
	m.token = token
	m.plan = pluginmigrate.BuildPlan(m.scan, avail)
	m.panel = panelReview
	m.reviewApply = true
	m.tokenInput.Blur()
	return m, nil
...
func (m *Model) viewReview() (title, status, body string) {
	...
	if m.plan.AddStoreRepository {
		b.WriteString(tui.DimStyle.Render("  • add Composer repository "+pluginmigrate.StoreRepositoryURL) + "\n")
	}
...
type runState struct {
	states    map[pluginmigrate.StepID]pluginmigrate.StepState
	log       []string
	events    <-chan pluginmigrate.StepEvent
	finished  bool
	succeeded bool
	err       error
...
func (m *Model) beginRun() (app.Content, tea.Cmd) {
	...
	m.panel = panelRun
	m.run = runState{
		states: make(map[pluginmigrate.StepID]pluginmigrate.StepState),
		cancel: cancel,
...
func (m *Model) updateRun(msg tea.Msg) (app.Content, tea.Cmd) {
	switch msg := msg.(type) {
	case runEventMsg:
		ev := pluginmigrate.StepEvent(msg)
		switch {
		case ev.Line != "":
			m.run.log = tui.AppendTail(m.run.log, runLogKeep, ev.Line)
		case ev.Step == pluginmigrate.StepFinished:
			m.run.finished = true
			m.run.succeeded = ev.State == pluginmigrate.StateOK
			m.run.err = ev.Err
		default:
...
func (m *Model) viewRun() (title, status, body string) {
	...
	items := make([]tui.StepItem, 0, len(pluginmigrate.RunSteps))
	for _, step := range pluginmigrate.RunSteps {
		item := tui.StepItem{Label: step.Label()}
		switch m.run.states[step] {
		case pluginmigrate.StateRunning:
			item.State = tui.StepStateActive
		case pluginmigrate.StateOK, pluginmigrate.StateWarn:
			item.State = tui.StepStateDone
		case pluginmigrate.StateFail:
			item.Label = failStyle.Render(step.Label())
			item.State = tui.StepStateDone
		case pluginmigrate.StatePending:
	...
	visible := m.frameHeight() - len(pluginmigrate.RunSteps) - 8
	if visible < 3 {
		visible = 3
	}
...
func (m *Model) viewDone() string {
	...
	if m.done.succeeded {
		...
		if n := m.plan.Count(pluginmigrate.ActionPathRepository); n > 0 {
